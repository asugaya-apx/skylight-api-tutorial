...
async function main() {
	...
	async function onMessageCallback(messageData){
		...
		if(messageData.eventType !== "cards")return;
      	...
      	if(updatedComponent.componentType === "capturePhoto") {
      		//Captured photos are stored in results
	...
	for(let user of selectGroup.members){
		...
		const assignmentInfo = await client.apiPost("/assignment/assignments", assignmentData);
		console.log("Pushed assignment to " + user.username);

		/*** Tutorial: 8-dynamicUpdates ***/
		//In a production environment, the assignment/sequence/card ids should be stored in classes, instead of finding the card as we do below
		const assignmentId = assignmentInfo.id;
		const sequenceId = assignmentData.sequences[0].id;
		const cardId = assignmentData.sequences[0].cards[0].id;
		const cardUrl = `/assignment/assignments/${assignmentId}/sequences/${sequenceId}/cards/${cardId}`;
		const GREETING_STRINGS = ["Hallo", "Konnichiwa", "Bonjour", "Guten Tag", "Hola"];

		async function updateGreetings(helloIndex){
			const cardData = await client.apiGet(cardUrl);
			cardData.layout.text = GREETING_STRINGS[helloIndex];
			await client.apiPut(cardUrl, cardData);

			helloIndex += 1;
			helloIndex %= GREETING_STRINGS.length;

			setTimeout(function(){updateGreetings(helloIndex);}, 2000);
		}

		//Call our function once to get the ball rolling
		updateGreetings(1);
